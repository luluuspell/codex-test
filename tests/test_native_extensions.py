...
def make_tasks():
    world = WorldModel(); events = EventStore(); return TaskRuntime(world, events)


def test_control_intent_never_confuses_stop_speaking_with_task_cancel():
    tasks = make_tasks()
    task = tasks.create("long work", ("done",)); task.state = TaskState.RUNNING
...
def test_control_resolver_requests_user_when_multiple_tasks_match():
    tasks = make_tasks()
    a = tasks.create("a", ("done",)); b = tasks.create("b", ("done",))
...
def test_policy_requires_confirmation_for_external_and_destructive():
    engine = PolicyEngine({"ws": WorkspacePolicy(allowed_capabilities=frozenset({"web"}))})
...
def test_memory_refuses_unconfirmed_global_inference():
    store = MemoryStore()
    candidate = MemoryCandidate(MemoryKind.FACT, "dark_theme", True, "global", ("e1",), 0.8, False)
...
def test_memory_revision_supersedes_without_destroying_old_fact():
    store = MemoryStore()
    r1 = store.commit(MemoryCandidate(MemoryKind.FACT, "price", 299, "project:store", ("e1",), 1.0, True))
    r2 = store.commit(MemoryCandidate(MemoryKind.FACT, "price", 319, "project:store", ("e2",), 1.0, True))
...
def test_strategy_requires_accumulated_support_before_promotion():
    store = MemoryStore()
    r = store.commit(MemoryCandidate(MemoryKind.STRATEGY, "vite_deploy", "build_first", "project:web", ("e1",), 0.7))
    assert r.strategy_state is StrategyState.CANDIDATE
    store.add_strategy_support(r.memory_id)
    assert r.strategy_state is StrategyState.SUPPORTED
    store.add_strategy_support(r.memory_id)
    assert r.strategy_state is StrategyState.PROMOTED
...
def test_fly_consumer_reports_lag_without_blocking_engine_state():
    ...
    fly.receive(120); fly.processed(100)

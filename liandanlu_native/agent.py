...
from .integrity import TERMINAL_TASKS, UNSETTLED, OperationConflict, StateConflict
...
class NativeAgentRunner:
    ...
    def step(self, task: Task, manifest: ContextManifest, *, task_lease: TaskLease | None = None) -> None:
        ...
        # A stale worker must not call a paid model or overwrite the new owner's Task.
        ...
        try:
            ...
            if decision is not PolicyDecision.ALLOW:
                blocked = decision is PolicyDecision.DENY
                update_owned(self.tasks, task, task_lease,
                    state=TaskState.BLOCKED if blocked else TaskState.WAITING,
                    reason='policy:denied' if blocked else 'policy:confirmation_required',
                    event_type='agent.action.blocked' if blocked else 'agent.action.confirmation_required',
                    actor='policy')
                return
            ...
            try:
                op = self.operations.prepare(task, proposal, expected_revisions=expected,
                                             task_lease=task_lease, resource_lease=resource_lease)
                ...
                # A user control may have committed while the model was thinking.
                if self.tasks.persistence is not None:
                    self.tasks.refresh(task.task_id)
                if self._control_pending(task):
                    return
                raise
            # Do not catch LeaseLost by writing WAITING over the new owner's task.
            check_owner(self.tasks, task.task_id, task_lease)
            if self.tasks.persistence is not None:
                self.tasks.refresh(task.task_id)
            if self._control_pending(task):
            ...
            # Broker also checks the durable operation, so stale memory cannot release an active reservation.
